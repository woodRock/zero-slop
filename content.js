...
const SLOP_HEURISTICS = {
  hardBlocks: [
    { regex: /\b(stop|still)\s+paying\s+for\b.{0,30}(storage|icloud|gmail|subscription)/gi, label: "Subscription Hook", description: "Targets your exhaustion with recurring fees to sell a 'magic button' solution." },
    { regex: /\bI\s+(found|discovered)\s+a\s+(way|secret|tool)\b/gi, label: "Curiosity Gap", description: "Claims of secret knowledge used to farm engagement." },
    { regex: /\b(I\s+)?hope\s+this\s+helps\s+you\s*↓/gi, label: "Engagement Ending", description: "Standardized call-to-action used in slop threads." },
    { regex: /\bstop\s+(declining|rejecting)\s+spam\s+calls\b/gi, label: "Fear-Based Hook", description: "Targets common annoyances with unverified solutions." },
    { regex: /\bif\s+your\s+iphone\s+gets\s+stolen\b/gi, label: "Fear-Based Hook", description: "Exploits anxiety about device security to extract attention." },
    { regex: /\b(most|9[59]% of)\s+(developers|students|people)\s+are\s+using\b.{0,30}\bwrong\b/gi, label: "Competence Gap", description: "Weaponizes the fear that you are falling behind your peers." },
    { regex: /\bsomeone\s+(built|leaked|compiled|just)\s+a\s+(tool|skill|system|repo|skill)\b/gi, label: "Third-Party Validation", description: "Uses fake institutional credibility to launder low-quality content." },
    { regex: /\b(your|a)\s+\$[\d,]+\s+camera\b.{0,50}\blosing\b/gi, label: "Comparison Hook", description: "Uses absurd gear comparisons to create scroll-stopping curiosity." },
    { regex: /\b(vibe\s+coding|claude\s+code|openclaw)\b/gi, label: "Trending Slop", description: "High-volume industrial farming of current AI keywords." },
    { regex: /\b(don.t|do\s+not)\s+change\s+the\s+iphone\b/gi, label: "Fear-Based Hook", description: "Uses fear of tech obsolescence to capture your scroll-speed." },
    { regex: /\bbest\s+for\s+(logic|writing|research|video)\b/gi, label: "Comparison Table", description: "Standardized AI model comparison templates used in slop." },
    { regex: /\bfollow\s+(me|@\w+)\b/gi, label: "Engagement Gate", description: "Clear signal of industrial audience capture." },
    { regex: /must follow/gi, label: "Engagement Gate", description: "Forced following to bypass algorithmic filters." },
    { regex: /\bcomment\b.{0,30}\bto (get|receive|join|access)\b/gi, label: "Comment Bait", description: "Tricks the algorithm into seeing organic engagement." },
    { regex: /\b(retweet|like\s+and\s+retweet)\b/gi, label: "Engagement Loop", description: "The most basic form of algorithmic currency extraction." },
    { regex: /\bbookmark\s+(this|it|now|thread|post)\b/gi, label: "Save/Bookmark Bait", description: "Exploits Miller's Law by overloading you with info to process now." },
    { regex: /\bsave\s+this\b/gi, label: "Save/Bookmark Bait", description: "Designed to increase post dwell-time and algorithmic 'saves'." },
    { regex: /\b(dm\s+me|dm\s+for|send\s+me\s+(a\s+)?dm)\b/gi, label: "Direct-Funnel Loop", description: "Moves the extraction into private messages to bypass public scrutiny." },
    { regex: /\$[\d,]+\+?\s*\/\s*(mo|month|day|week|hr|hour|year)/gi, label: "Income Claim", description: "Unverified financial promise designed to bypass logical skepticism." },
    { regex: /\$[\d,]+[k]?\s+per\s+(mo|month|day|week|hour)/gi, label: "Income Claim", description: "Specific, high-value financial hooks used by slop factories." },
    { regex: /\$[\d,]+.{0,40}(replac|instead of|consultant|lawyer|doctor|agency|degree|analyst|designer|copywriter)/gi, label: "Income Claim", description: "Claims that AI can replace high-value human professional services." },
    { regex: /\b(make|earn|generate|earning|making)\b.{0,20}\$[\d,]+/gi, label: "Income Claim", description: "Direct promises of unverified monetary gain." },
    { regex: /\b(money machine|cash machine|print money|income machine)\b/gi, label: "Hustle Buzzword", description: "Dehumanizes wealth generation into a mechanical, low-effort process." },
    { regex: /\bside\s+(hustle|income)\b/gi, label: "Aspirational Trap", description: "Targets workers with a fantasy of escaping the system." },
    { regex: /\bdata entry\b/gi, label: "Hustle Buzzword", description: "A common 'passive income' trope used to farm engagement." },
    { regex: /\bhere are\s+[1-9]\d*\b/gi, label: "List Formula", description: "Uses specific numbers to hack your brain's internal spam filter." },
    { regex: /\b[1-9]\d*\s+prompts?\b/gi, label: "Prompt List Formula", description: "Mass-produced prompt collections with low actual utility." },
    { regex: /\b[1-9]\d*\s+(tools?|hacks?|tricks?|ways?|tips?|secrets?|mistakes?|steps?)\b/gi, label: "List Formula", description: "Designed to provide 'value' that is just below the fold." },
    { regex: /\b(use these|try these|steal these|copy these)\b.{0,20}\b(prompts?|tools?|tricks?)\b/gi, label: "List Formula", description: "Encourages low-effort copy-pasting over genuine learning." },
    { regex: /\bact (as|like) (a |an )?(professional|expert|senior|world-class|harvard)/gi, label: "Institutional Launder", description: "Steals the credibility of elite brands to launder low-quality content." },
    { regex: /\bstep[\s-]by[\s-]step\b/gi, label: "List Formula", description: "Makes complex technology seem simple for the unqualified." },
    { regex: /\b[1-9]\d*\+?\s*free\s+(ai\s+)?courses?\b/gi, label: "Lead Magnet", description: "The 'value' is bait for a downstream product or newsletter." },
    { regex: /\bBREAKING\b/g, label: "Clickbait Opener", description: "Artificially creates urgency to capture your initial scroll-speed." },
    { regex: /\bGOODBYE\b/g, label: "Clickbait Opener", description: "Uses finality to create a curiosity gap." },
    { regex: /\bR\.?I\.?P\.?\b/gi, label: "Clickbait Opener", description: "Uses fake finality to harvest engagement." },
    { regex: /\bSTOP (telling|using|doing|saying)\b/g, label: "Clickbait Opener", description: "Weaponizes authority to command your attention." },
    { regex: /\bCANCELLED\b.{0,60}(chatgpt|netflix|spotify|prime|subscription)/gi, label: "Clickbait Opener", description: "Exploits the fear of missing out on a 'magic button' solution." },
    { regex: /\b(most people don.t know|nobody (talks|is talking) about|very few (know|people)|hardly anyone|95% of people|99% of people)\b/gi, label: "Manufactured Consensus", description: "Weaponizes the fear that your peers are getting ahead of you." },
    { regex: /\bfor free\b/gi, label: "Lead Magnet", description: "Content is bait for a course blueprint or tool referral link." },
    { regex: /\bfaceless\b/gi, label: "Hustle Buzzword", description: "Promotes industrial content production without genuine intent." },
    { regex: /passive income/gi, label: "Hustle Buzzword", description: "Bait for a course blueprint or tool referral link." },
    { regex: /\bno (experience|skills?|coding|degree|team|budget|camera|luck)\b/gi, label: "Hustle Buzzword", description: "Lowers your critical guard by making success seem frictionless." },
    { regex: /\bzero to.{0,30}(income|money|\$)\b/gi, label: "Aspirational Trap", description: "The transformation story applied to a product or course funnel." },
    { regex: /\b(forget|ditch|quit|goodbye|replace)\s+chatgpt\b/gi, label: "Clickbait Opener", description: "Claims that all human work is replaceable by current AI wrappers." },
    { regex: /\b(giveaway|giving away)\b/gi, label: "Fake Giveaway", description: "Often a front for bot-network expansion." },
    { regex: /\b(prize|reward).{0,30}(follow|retweet|like|comment|enter)/gi, label: "Engagement Gate", description: "Engagement wall to harvest your account information." },
    { regex: /\bfree for \d+\s*hours?\b/gi, label: "Scarcity Bait", description: "Exploits the fear of missing out by creating fake time-pressure." },
    { regex: /\b(limited (spots?|seats?)|only \d+ spots?)\b/gi, label: "Scarcity Bait", description: "Uses specific, random numbers to make a scam look legitimate." },
    { regex: /\b(paid courses?).{0,30}free\b/gi, label: "Lead Magnet", description: "Fake pricing anchor to make slop look like a high-value gift." },
    { regex: /\ball paid.{0,20}free\b/gi, label: "Lead Magnet", description: "Designed to bypass your logical skepticism with 'free' value." },
    { regex: /\b(blueprint|masterclass|cheatsheet|playbook).{0,40}(free|get|dm|comment|follow)\b/gi, label: "Course Funnel", description: "Bait for a downstream product or tool referral link." },
    { regex: /\b(course|ebook|pdf).{0,40}\$[\d,]+\b/gi, label: "Course Funnel", description: "Direct monetization of engagement via informational products." },
    { regex: /\bgrok.{0,20}imagine\b/gi, label: "AI Promo", description: "Promotes AI tools using industrial engagement farming." },
    { regex: /\b(apob|pollo ai|seedance|heygen|synthesia)\b/gi, label: "AI Promo", description: "Specific tool promotions commonly found in slop networks." },
    { regex: /\bai\s+ugc\b/gi, label: "AI Promo", description: "Industrial content production disguised as organic user content." },
    { regex: /\bupload.{0,20}(photo|video|image).{0,40}(generate|create|make|turn into)\b/gi, label: "AI Promo", description: "Promotes AI creation tools through low-effort templates." }
  ],
  softSignals: [
